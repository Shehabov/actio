...
| `actio-design-system` | Step 1, before you write a component, and step 4 against your diff. It is the platform: shell anatomy and its breakpoint collapse, the inversion rule and its accent budget, metric tiles with no chrome, the card taxonomy, alert rows as a left accent rule at radius 0, queue rules, chart series, and the mobile-first translation table. Build what it specifies. Where the design spec and this system disagree, reject to `ux-designer` rather than picking one. |
